"""Sweep DWN across the F4RM 21-dataset suite + the 10 datasets DWN paper Table 5
evaluates on (where they don't already overlap).

DWN training is gradient-based (Adam + EFD). On CPU each cell costs from
~0.1 s (Iris) to ~30 s (Magic Gamma). Total budget for 31 datasets × 6 cells ×
3 seeds with the small grid is ~30–40 minutes.

For datasets with > 20,000 training samples we subsample down to 20 k via
``train_test_split(stratify=y, random_state=seed)`` before fitting, to keep the
sweep tractable on CPU. This is documented in the row's ``note``.
"""
...
N_SEEDS = 3
SYSTEM = "DWN"
MEM_METHOD = "dwn_binarized_lut"
MAX_TRAIN_SAMPLES = 20_000  # subsample any larger train set down to this cap

# F4RM 21-dataset suite
F4RM_SPECS = [
...
# DWN paper Table 5 datasets (OpenML IDs, excluding Segment which is in F4RM_SPECS)
DWN_PAPER_SPECS = [
    ("phoneme", "openml", 1489),
...
def main():
    ...
    df = load_cache()
    # Drop the placeholder paper-cited DWN row before adding measured rows
    placeholder_mask = (df["system"] == SYSTEM) & (df["seed"] == -1)
    if placeholder_mask.any():
        df = df[~placeholder_mask].reset_index(drop=True)
        save_cache(df)
        print(f"[{time.strftime('%H:%M:%S')}] Removed {placeholder_mask.sum()} placeholder DWN row(s).")
    print(f"[{time.strftime('%H:%M:%S')}] Cache: {len(df)} rows; {(df['system']==SYSTEM).sum()} DWN measured already.")

    grid = grid_dwn()
    print(f"Grid: {len(grid)} cells; {N_SEEDS} seeds; {len(ALL_SPECS)} datasets "
          f"(21 F4RM + 10 DWN-paper).")
    ...
    for ds_name, source, ident in ALL_SPECS:
        ...
        for cell_idx, hp in enumerate(grid):
            ...
            for s in range(N_SEEDS):
                ...
                # Stratified 80/20 split on full data
                Xtr, Xte, ytr, yte = train_test_split(
                    X, y, test_size=0.2, stratify=y, random_state=seed,
                )
                # Subsample training set if too big (DWN on CPU)
                subsample_note = ""
                if len(Xtr) > MAX_TRAIN_SAMPLES:
                    ...
                    subsample_note = f"DWN train subsampled to {MAX_TRAIN_SAMPLES} (CPU budget)"
        ...
        dwn_rows = (df["system"] == SYSTEM).sum()
        print(f"[{time.strftime('%H:%M:%S')}]   ...done {ds_name}: total {dwn_rows} DWN rows; "
    ...
    print(f"[{time.strftime('%H:%M:%S')}] Sweep complete: {appended} new rows; "
          f"total {(df['system']==SYSTEM).sum()} DWN rows; "
